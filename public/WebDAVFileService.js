...
/*global window eclipse:true XMLSerializer DOMParser XMLHttpRequest define BlobBuilder*/
...
define(["orion/Deferred"], function(Deferred) {
	...
	function parseDAV_propstat(propstatElement) {
		...
		for (var i = 0; i < propChildren.length; ++i) {
			var current = propChildren[i];
	...
	function parseDAV_response(responseElement) {
		var hrefElements = responseElement.querySelectorAll("href"),
			...
			statusElement, propstatElements, i,
			result = {
				href: []
			};
...
//			});
			...
			segments.forEach(function(parentName) {
		...
		fetchChildren: function(location) {
			...
			return _call("PROPFIND", location, {
				depth: 1
		...
		moveFile: function(sourceLocation, targetLocation, name) {
			...
			return _call("MOVE", sourceLocation, {
				Destination: target
			});
		},
		...
		copyFile: function(sourceLocation, targetLocation, name) {
			...
			return _call("COPY", sourceLocation, {
				Destination: target
			});
		},
		/**
	...
	if (window.Blob) {
		WebDAVFileService.prototype.readBlob = function(location) {
			return _call2("GET", location).then(function(result) {
				return result.response;
			});
		};

		WebDAVFileService.prototype.writeBlob = function(location, contents, args) {
			var headerData = {};
			if (args && args.ETag) {
				headerData["If-Match"] = args.ETag;
			}
