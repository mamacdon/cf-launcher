...
util.extend(ProcessManager.prototype, {
	...
	changeState: function(name, newState, callback) {
		// TODO clean up this method using promises
	...
	startApp: function(name, args, port, state) {
		...
		// Determine if the user app is Node.js. If it's something else-- eg. grunt, npm, some other command--
		// we are unable to pass the debug flags to the underlying Node process, if any. In such cases we can
		// only provide process control, not debugging :(
		var isNode = args[0] === "node" || /\/node$/.test(args[0]),
		    relaunchArgs = args.slice(1);
		if (isNode) {
			var isBreak = state === State.DEBUG_BREAK,
			    debugArg = isBreak ? "--debug-brk" : "--debug";
			args.splice(1, 0, debugArg); // insert --debug or --debug-brk argument after 'node'
		}
		...
		var launchFn = this.startApp.bind(this, name, relaunchArgs, port);
		var procInfo = this._spawn(args[0], args.slice(1), childopts, name, launchFn, state);
